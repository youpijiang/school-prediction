...
bar_labels = [f"{level_names[i]} ({lefts[i]:.1%}~{lefts[i]+widths[i]:.1%})" for i in range(len(level_names))]
...
# ==================== 坐标轴美化：隐藏无意义的 0% 和 0 ====================
# 定义不含 0 的百分比刻度（从 10% 到 100%，步长 10%）
pct_ticks = np.arange(0.1, 1.01, 0.1)
pct_labels = [f"{t:.0%}" for t in pct_ticks]

# 子图1 x轴
fig.update_xaxes(tickvals=pct_ticks, ticktext=pct_labels,
                 title_text="影响时间占比", row=1, col=1)
# 子图1 y轴（频数保留0，如需去掉0可额外设置，但0有实际意义）
fig.update_yaxes(title_text="频数", row=1, col=1)

# 子图2 x轴和y轴
fig.update_xaxes(tickvals=pct_ticks, ticktext=pct_labels,
                 title_text="影响占比", row=1, col=2)
fig.update_yaxes(tickvals=pct_ticks, ticktext=pct_labels,
                 title_text="累积概率", range=[0.05, 1.02], row=1, col=2)

# 子图3 x轴
fig.update_xaxes(tickvals=pct_ticks, ticktext=pct_labels,
                 title_text="影响占比区间", range=[0.05, 1.0], row=2, col=1)
fig.update_yaxes(title_text="", row=2, col=1)

# 子图4 x轴
fig.update_xaxes(tickvals=pct_ticks, ticktext=pct_labels,
                 title_text="影响占比", row=2, col=2)
